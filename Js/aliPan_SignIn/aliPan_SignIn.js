...
const aliYunPanRefreshTokenKey = 'AliYunPanRefreshTokenKey'
...
function ToolKit(scriptName, scriptId, options) {
    class Request {
        ...
        fetch(options, method = "GET") {
            ...
            const doFetch = new Promise((resolve, reject) => {
                fetcher.call(this, options, (error, response, data) => error ? reject(error) : resolve({
                    error: error,
                    response: response,
                    data: data
                }))
            });
            const delayFetch = (promise, timeout = 5e3) => {
    ...
    return new class {
        constructor(scriptName, scriptId, options) {
            ...
            this.preTgEscapeCharMapping = {"|`|": ",backQuote,"};
            this.finalTgEscapeCharMapping = {",backQuote,": "`", "%2CbackQuote%2C": "`"};
            this.tgEscapeCharMapping = {_: "\\_", "*": "\\*", "`": "\\`"};
        ...
        msg(subtitle, message, openUrl, mediaUrl, copyText, autoDismiss) {
            ...
            if (this.isEnableTgNotify) {
                ...
                if (this.isSurge() || this.isLoon() || this.isStash()) {
                    ...
                    $push.schedule({title: this.name, body: subtitle ? subtitle + "\n" + message : message})
                }
            }
        }
        ...
        updateBoxjsSessions(key, val) {
            ...
            if (!isExists) {
                curSessionDatas.push({key: key, val: val})
            }
            boxjsSessions.forEach(session => {
                if (session.id == curSessionId) {
